...
class BuildNoSQLDatabase:
    ...
    def get_province_unemployed_stats(self, year):
        ...
        for prov in provinces:
            prov = prov[0]
            total_people = self.__session.query(classes[year].personnr).filter_by(prov=prov).count()
            total_unemployed = self.__session.query(classes[year].uqnr).filter_by(prov=prov).filter_by(employ_status='unemployed').count()
            graph_data[prov] = {'total_people': total_people, 'total_unemployed': total_unemployed}
    ...
    def get_unemployment_by_education_level(self, year):
        ...
        for level in education_levels:
            level = level[0]
            total_people = self.__session.query(classes[year].personnr).filter_by(education=level).count()
            ...
            graph_data[level] = {'total_people': total_people, 'total_unemployed': total_unemployed}
